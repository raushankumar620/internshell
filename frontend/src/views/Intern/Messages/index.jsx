  ...
  const fetchConversations = async (showRefresh = false) => {
    try {
      ...
      if (response.success) {
        ...
        if (validConversations.length > 0 && !selectedChat) {
  ...
  const ConversationItem = ({ conv, isSelected, onClick }) => (
    ...
    >
      <ListItemAvatar>
        ...
        >
          <Avatar 
            src={conv.user?.avatar || conv.user?.companyLogo}
            sx={{ 
              bgcolor: getAvatarColor(conv.user?.companyName || conv.user?.name),
              width: 48,
              height: 48,
              fontSize: '1.1rem',
              fontWeight: 600,
          ...
          >
            {(conv.user?.companyName || conv.user?.name)?.charAt(0)?.toUpperCase() || conv.user?.email?.charAt(0)?.toUpperCase() || '?'}
          </Avatar>
        </Badge>
      </ListItemAvatar>
      <ListItemText
        ...
        primary={
          <Box display="flex" alignItems="center" justifyContent="space-between">
            ...
            >
              {conv.user?.companyName || conv.user?.name || conv.user?.email || 'Unknown'}
            </Typography>
            <Typography 
              variant="caption" 
              sx={{ 
        ...
        secondary={
          <Box display="flex" alignItems="center" gap={0.5} mt={0.3}>
            {conv.user?.role === 'employer' && conv.user?.name && (
              <>
                <BusinessIcon sx={{ fontSize: 12, color: 'primary.main' }} />
                <Typography 
                  variant="caption" 
                  sx={{
                    color: 'primary.main',
            ...
            <Box sx={{ flex: 1, overflow: 'auto' }}>
              {loading ? (
                ...
                <EmptyState
                  ...
                  subtitle="Start a conversation with employers to see them here"
                />
              ) : (
                <List sx={{ p: 0 }}>
            ...
            {selectedChat ? (
              <>
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    ...
                    >
                      <Avatar 
                        src={selectedChat?.avatar || selectedChat?.companyLogo}
                        sx={{ 
                          bgcolor: getAvatarColor(selectedChat?.companyName || selectedChat?.name),
                          width: 44,
                          height: 44
                        }}
                      >
                        {(selectedChat?.companyName || selectedChat?.name)?.charAt(0)?.toUpperCase() || '?'}
                      </Avatar>
                    </Badge>
                    <Box>
                      <Typography variant="subtitle1" fontWeight={600}>
                        {selectedChat?.companyName || selectedChat?.name || selectedChat?.email || 'Unknown'}
                      </Typography>
                      <Box display="flex" alignItems="center" gap={0.5}>
                        {selectedChat?.role === 'employer' ? (
                          <Chip
                            icon={<BusinessIcon sx={{ fontSize: '14px !important' }} />}
                            label={selectedChat?.name || 'Employer'}
                            size="small"
                            sx={{
                              height: 20,
                              fontSize: '0.65rem',
              ...
              >
                ...
                <Typography variant="body1" color="text.disabled" textAlign="center" sx={{ maxWidth: 350 }}>
                  Choose a conversation from the list to view messages and start chatting with employers
                </Typography>
              </Box>
            )}
          </Box>
